...
class BedrockChatRouter:
    ...
    def chat_with_agent(self, message: str):
        ...
        # Keeping session history for up to 300 KB (400KB is the record size limit in DDB)
        if self.__get_size_in_kb(self.chat_context.history) > 300:
            while self.__get_size_in_kb(self.chat_context.history) > 300 and len(self.chat_context.history) > 1:
                # Remove the oldest message (first item in the list)
                self.chat_context.history.pop(0)
