...
def merge_files(template_dir, template_file, table_file, schema_file, query_file, prompt_output):
    try:
        template_dir = os.path.join(os.getcwd(), template_dir, 'templates')
        ...
        logger.info(f"Prompt successfully generated: {prompt_output}")
    except Exception as e:
        logger.error(f"An error occurred: {e}")
        raise

# Set template_dir based on first input argument to python code
if len(sys.argv) > 1:
        arguments = sys.argv[1:]
        template_dir = arguments[0]
        merge_files(
            ...
            prompt_output=f'{template_dir}/orchestration_prompt.txt'
        )
else:
    raise Exception("No arguments (template_dir) provided.")
