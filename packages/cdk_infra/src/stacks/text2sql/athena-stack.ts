...
export class AthenaStack extends Stack {
  ...
  public readonly ECOMMERCE_DATA_DATABASE: glue.CfnDatabase;
  ...
  constructor(scope: Construct, id: string, props: AthenaStackProps) {
    ...
    new s3deploy.BucketDeployment(this, "DeployProductsSampleTable", {
      sources: [
        s3deploy.Source.asset(path.join(__dirname, "..", "..", "assets", "ecommerce_data", "products")),
      ],
      destinationBucket: this.ATHENA_DATA_BUCKET,
      destinationKeyPrefix: "ecommerce_data/products",
    });

    // Upload a sample csv file for reviews table
    new s3deploy.BucketDeployment(this, "DeployReviewsSampleTable", {
      sources: [
        s3deploy.Source.asset(path.join(__dirname, "..", "..", "assets", "ecommerce_data", "reviews")),
      ],
      destinationBucket: this.ATHENA_DATA_BUCKET,
      destinationKeyPrefix: "ecommerce_data/reviews",
    });

    // Create Athena Database
    this.ECOMMERCE_DATA_DATABASE = new glue.CfnDatabase(this, "EcommerceDataDatabase", {
    ...
    new glue.CfnTable(this, "ProductsTable", {
      ...
      tableInput: {
        ...
        storageDescriptor: {
          ...
          location: `s3://${this.ATHENA_DATA_BUCKET.bucketName}/ecommerce_data/products`,
    ...
    new glue.CfnTable(this, "ReviewsTable", {
      ...
      tableInput: {
        ...
        storageDescriptor: {
          ...
          location: `s3://${this.ATHENA_DATA_BUCKET.bucketName}/ecommerce_data/reviews`,
          inputFormat: "org.apache.hadoop.mapred.TextInputFormat",
          outputFormat: "org.apache.hadoop.hive.ql.io.HiveIgnoreKeyTextOutputFormat",
          serdeInfo: {
            serializationLibrary: "org.apache.hadoop.hive.serde2.lazy.LazySimpleSerDe",
    ...
    new CfnOutput(this, "EcommerceDataDatabaseName", {
      value: this.ECOMMERCE_DATA_DATABASE.ref,
      description: "EcommerceData Database Name",
    });
  }
}
